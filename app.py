...
@app.route('/api/search_groups_for_user/<username>')
@require_auth
@require_api_permission(action='can_manage_groups')
def api_search_groups_for_user(username):
    query = request.args.get('query', '')
    if not query or len(query) < 3:
        return jsonify([])
    ...
    try:
        ...
        current_group_dns = {dn.lower() for dn in user.memberOf.values} if 'memberOf' in user and user.memberOf.values else set()
        ...
        available_groups = [
            {
                'cn': get_attr_value(group, 'cn'),
                'description': get_attr_value(group, 'description'),
            }
            for group in conn.entries
            if group.distinguishedName.value.lower() not in current_group_dns
        ]
        return jsonify(available_groups)
    except Exception as e:
        logging.error(f"Erro na API de busca de grupos para o usuário '{username}': {e}", exc_info=True)
        return jsonify({'error': str(e)}), 500

@app.route('/api/add_user_to_group/<username>/<group_name>', methods=['POST'])
@require_auth
@require_api_permission(action='can_manage_groups')
def api_add_user_to_group(username, group_name):
    try:
        conn = get_service_account_connection()
        ...
        if conn.result['description'] == 'success':
            logging.info(f"Usuário '{username}' adicionado ao grupo '{group_name}' por '{session.get('ad_user')}'.")
            return jsonify({'success': True, 'message': 'Usuário adicionado com sucesso.'})
        else:
            logging.error(f"Falha ao adicionar '{username}' ao grupo '{group_name}': {conn.result['message']}")
            return jsonify({'error': f"Falha ao adicionar usuário: {conn.result['message']}"}), 500
    except Exception as e:
        logging.error(f"Erro ao adicionar '{username}' ao grupo '{group_name}': {e}", exc_info=True)
        return jsonify({'error': str(e)}), 500

@app.route('/api/remove_user_from_group/<username>/<group_name>', methods=['POST'])
@require_auth
@require_api_permission(action='can_manage_groups')
def api_remove_user_from_group(username, group_name):
    try:
        conn = get_service_account_connection()
        user_to_remove = get_user_by_samaccountname(conn, username, ['distinguishedName'])
        group_to_modify = get_group_by_name(conn, group_name, ['distinguishedName'])

        if not user_to_remove or not group_to_modify:
            return jsonify({'error': 'Usuário ou grupo não encontrado.'}), 404
        ...
        if conn.result['description'] == 'success':
            logging.info(f"Usuário '{username}' removido do grupo '{group_name}' por '{session.get('ad_user')}'.")
            return jsonify({'success': True, 'message': 'Usuário removido com sucesso.'})
        else:
            logging.error(f"Falha ao remover '{username}' do grupo '{group_name}': {conn.result['message']}")
            return jsonify({'error': f"Falha ao remover usuário: {conn.result['message']}"}), 500
    except Exception as e:
